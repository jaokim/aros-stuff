...
#elif __amigaos4__
static uint32 CancelHandlerFunc(struct Library * SysBase, uint32 signals, struct sigaction *data)
{
#else
...
static void StarterFunc(void)
{
    ...
    if (!setjmp(inf->jmp))
    {
        ...
        if (inf->attr.stackaddr != NULL && inf->attr.stacksize > 0)
        {
            ...
            #else
            DB2(bug("%s() doing stackswap stuff\n", __FUNCTION__));
            //struct StackSwapArgs swapargs;
            struct StackSwapStruct stack;
            int(*func)(void*);
            //swapargs.Args[0] = (IPTR)inf->arg;
            ...
            func = inf->start;
            StackSwap(&stack);
            //NewStackRun(inf->start,
            inf->ret = (void*)func(inf->arg);
            StackSwap(&stack);
            #endif
        }
        else
        {
            DB2(bug("%s() calling code\n", __FUNCTION__));
            inf->ret = inf->start(inf->arg);
        }
    } else {
        DB2(bug("%s() setjmp fail\n", __FUNCTION__));
    }
...
void (*initfunc)(void) SECTION(".ctors") =_Init_Func;
void (*exitfunc)(void) SECTION(".dtors") =_Exit_Func;
#endif
